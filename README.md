# 股票分析系统 (Stock Analyzer)

一个功能完整的本地股票分析系统，支持A股、美股和ETF的数据获取、技术分析、策略回测和交易信号生成。

## ✨ 功能特性

- 📊 **多市场支持**: A股、美股、ETF
- 📈 **技术指标**: MACD、RSI、KDJ、布林带、均线等
- 🎯 **策略交易**: **11种**经典量化策略
- 📉 **回测系统**: 基于VectorBT的高性能回测
- 💾 **本地存储**: SQLite数据库存储历史数据
- 📊 **可视化**: K线图、技术指标图、资金曲线
- ⚡ **增量更新**: 自动检测并下载最新数据
- 🌐 **Web界面**: 基于Streamlit的交互式分析界面

## 🚀 快速开始

### 安装依赖

```bash
pip install -r requirements.txt
```

### 运行方式

#### 方式1: Web交互界面（推荐）

```bash
# 启动Web应用
python3 -m streamlit run web/app.py

# 或使用启动脚本
cd web && ./start.sh
```

然后打开浏览器访问: http://localhost:8501

#### 方式2: 命令行模式

```bash
python main.py
```

#### 方式3: 批量分析

```bash
python examples/batch_analysis.py
```

## 📁 项目结构

```
stock-analyzer/
├── config/              # 配置文件
│   ├── settings.yaml    # 全局配置
│   └── strategies.yaml  # 策略配置
├── data/                # 数据存储
│   ├── raw/            # 原始数据
│   └── processed/      # 处理后数据
├── database/           # 数据库模块
│   └── db_manager.py   # SQLite管理
├── fetcher/            # 数据获取模块
│   ├── base_fetcher.py    # 基础接口
│   ├── akshare_fetcher.py # A股/ETF数据
│   └── yfinance_fetcher.py # 美股数据
├── analysis/           # 分析模块
│   └── indicators.py   # 技术指标计算
├── strategy/           # 策略模块
│   ├── base_strategy.py   # 策略基类
│   ├── ma_strategy.py     # 均线策略
│   ├── macd_strategy.py   # MACD策略
│   ├── rsi_strategy.py    # RSI策略
│   └── engine.py          # 策略引擎
├── backtest/           # 回测模块
│   └── engine.py       # 回测引擎
├── visualization/      # 可视化模块
│   └── charts.py       # 图表绘制
├── utils/              # 工具模块
│   ├── config.py       # 配置管理
│   └── helpers.py      # 辅助函数
├── main.py             # 主入口
└── requirements.txt    # 依赖列表
```

## 🔧 核心功能

### 1. 数据获取

```python
from stock_analyzer import StockAnalyzer

analyzer = StockAnalyzer()

# 获取A股数据
df = analyzer.fetch_and_store("000001", market="A")

# 获取美股数据
df = analyzer.fetch_and_store("AAPL", market="US")
```

### 2. 技术分析

```python
# 分析股票并获取交易信号
result = analyzer.analyze("000001", market="A")

print(f"综合信号: {result['final_signal']}")
print(f"置信度: {result['confidence']}")

# 各策略详情
for name, detail in result['details'].items():
    print(f"{name}: {detail['signal']} - {detail['reason']}")
```

### 3. 策略回测

```python
# 回测MACD策略
backtest_result = analyzer.backtest(
    symbol="000001",
    strategy_name="macd",
    market="A",
    start_date="2022-01-01",
    end_date="2024-01-01"
)

print(f"总收益率: {backtest_result['total_return_pct']}")
print(f"最大回撤: {backtest_result['max_drawdown_pct']}")
print(f"交易次数: {backtest_result['total_trades']}")
```

### 4. 绘制图表

```python
# 生成分析图表
analyzer.plot_analysis("000001", save_dir="output/charts")
```

## 📊 支持的策略（11种）

### 趋势跟踪策略

| 策略名称 | 说明 | 风险等级 |
|---------|------|---------|
| **双均线交叉** | 短期均线上穿长期均线买入，下穿卖出 | 中 |
| **MACD** | MACD金叉买入，死叉卖出 | 中 |
| **动量策略** | 追涨杀跌，基于价格和成交量动量 | 高 |
| **突破策略** | 突破近期高低点进行交易 | 高 |

### 均值回归策略

| 策略名称 | 说明 | 风险等级 |
|---------|------|---------|
| **RSI超买卖** | RSI超卖买入，超买卖出 | 中 |
| **布林带突破** | 价格突破上轨买入，跌破下轨卖出 | 中 |
| **均值回归** | 价格偏离均值后回归，适合震荡市场 | 中 |
| **KDJ随机指标** | 基于KDJ指标的交叉和超买超卖 | 中 |

### 其他策略

| 策略名称 | 说明 | 风险等级 |
|---------|------|---------|
| **成交量策略** | 基于成交量变化的量价分析 | 中 |
| **多因子组合** | 综合MA、MACD、RSI等多个因子 | 低 |
| **网格交易** | 在价格区间内低买高卖，适合震荡市场 | 低 |

📖 [查看详细策略说明](docs/STRATEGIES.md)

## ⚙️ 配置说明

### 全局配置 (config/settings.yaml)

```yaml
database:
  path: "data/stock_analyzer.db"  # 数据库路径

fetcher:
  akshare:
    rate_limit: 1.0  # 请求频率限制（秒）
    retry_times: 3   # 重试次数

update:
  start_date: "2020-01-01"  # 默认起始日期
  adjust: "qfq"            # 复权方式: qfq(前复权)/hfq(后复权)

backtest:
  initial_cash: 100000  # 初始资金
  commission: 0.0003    # 手续费率
  slippage: 0.001       # 滑点
```

### 策略配置 (config/strategies.yaml)

```yaml
strategies:
  ma_cross:
    enabled: true
    params:
      short_window: 5
      long_window: 20
    risk_management:
      stop_loss: 0.05    # 止损比例
      take_profit: 0.10  # 止盈比例
      max_position: 0.3  # 最大仓位
```

## 📈 技术指标

- **移动平均线 (MA)**: 5日、10日、20日、30日、60日
- **MACD**: DIF、DEA、柱状图
- **RSI**: 相对强弱指标
- **KDJ**: 随机指标
- **布林带**: 上轨、中轨、下轨
- **ATR**: 平均真实波幅
- **成交量MA**: 成交量移动平均线

## 🎯 使用示例

### 批量分析关注列表

```python
# 定义关注列表
watchlist = [
    ("000001", "A"),  # 平安银行
    ("000002", "A"),  # 万科A
    ("600519", "A"),  # 贵州茅台
    ("AAPL", "US"),   # 苹果
    ("TSLA", "US"),   # 特斯拉
]

# 获取所有信号
signals_df = analyzer.get_watchlist_signals(watchlist)
print(signals_df)
```

### 自定义策略参数

```python
from strategy.engine import StrategyEngine

engine = StrategyEngine()

# 使用自定义参数创建策略
strategy = engine.get_strategy('ma_cross', {
    'short_window': 10,
    'long_window': 30,
    'stop_loss': 0.03,
    'take_profit': 0.08
})
```

## ⚠️ 免责声明

本系统仅供学习研究使用，不构成任何投资建议。股市有风险，投资需谨慎。

- 历史回测结果不代表未来收益
- 交易信号仅供参考，请结合自身判断
- 实盘交易请自行承担风险

## 🖥️ Web界面功能

### 界面预览

Web界面提供以下功能：

1. **📊 实时行情显示**
   - 当前价格、涨跌幅
   - 今日最高/最低价
   - 成交量

2. **🎯 交易信号分析**
   - 综合信号（买入/卖出/观望）
   - 多策略信号对比
   - 置信度评分

3. **📈 技术分析图表**
   - 价格走势图（含均线）
   - MACD指标图
   - RSI指标图
   - 原始数据表格

4. **📉 策略回测**
   - 收益率统计
   - 最大回撤分析
   - 资金曲线图
   - 交易记录

### Web界面使用步骤

```bash
# 1. 启动Web服务
python3 -m streamlit run web/app.py

# 2. 在浏览器中打开 http://localhost:8501

# 3. 在侧边栏输入股票代码（如：000001）

# 4. 选择市场类型（A股/美股/ETF）

# 5. 点击"更新数据"获取最新行情

# 6. 查看分析结果和交易信号
```

### 界面截图

Web界面包含：
- **侧边栏**: 股票代码输入、市场选择、策略配置
- **主页面**: 实时行情卡片、交易信号、技术分析图表、回测结果

## 📝 依赖说明

主要依赖库：

- **akshare**: A股/港股/ETF数据获取
- **yfinance**: 美股数据获取
- **pandas**: 数据处理
- **vectorbt**: 回测引擎
- **mplfinance**: K线图绘制
- **matplotlib**: 图表绘制
- **streamlit**: Web应用框架

## 🤝 贡献

欢迎提交Issue和PR！

## 📄 许可证

MIT License

## 🙏 致谢

- [AKShare](https://www.akshare.xyz/) - 提供A股数据接口
- [yfinance](https://github.com/ranaroussi/yfinance) - 提供美股数据接口
- [vectorbt](https://vectorbt.dev/) - 提供回测引擎
